...
1. **ก่อน merge** — เวลา template repo กลางมีการอัปเดตไปเรื่อย ๆ (เช่น
   ...
   "ของจาก template" ไม่ใช่ฟีเจอร์ของตัวเอง สำหรับไฟล์ที่ไม่ใช่ config point
   ที่รู้จักอยู่แล้ว script จะพิมพ์เนื้อ diff เต็มให้ด้วย เพื่อให้ประเมินได้ว่า
   การเปลี่ยนแปลงเชิงโครงสร้าง/โค้ด (เช่น extend class ใหม่) กระทบกับโค้ดที่
   โปรเจกต์นี้ custom ไว้เองหรือไม่ ก่อนจะตัดสินใจ merge
...
- **เช็คไขว้ด้วยเนื้อไฟล์จริง** — เพราะ baseline ข้างบนเชื่อ commit message
  ล้วน ๆ ซึ่งอาจผิดได้ (เช่นโปรเจกต์นี้มี squash-merge PR ของตัวเองที่ทำให้
  commit message เดิม "X.Y.Z-template" หายไป ทั้งที่เนื้อไฟล์ merge มาแล้ว
  จริง) script จะเทียบไฟล์จริงของ HEAD กับทุก tag ของ template แล้วหาว่า
  tag ไหน diff เหลือน้อยที่สุด ถ้าผลไม่ตรงกับ baseline ที่เจอจาก commit
  message จะพิมพ์คำเตือนออกมา

**ถ้าเจอกรณีใดกรณีหนึ่งต่อไปนี้ — หา baseline จาก commit message ไม่เจอเลย,
หรือขึ้นคำเตือน `[ตรวจสอบเพิ่มเติม]` ว่าเนื้อไฟล์จริงไม่ตรงกับ baseline —
ห้ามเดาเอาเองหรือปล่อยผ่าน ให้ถามผู้ใช้ตรงๆ ก่อนเสมอ** เช่น "ล่าสุดที่คุณ
merge/อัปเดต template มาเป็นเวอร์ชันอะไร (ถ้าจำได้)" แล้วรัน script อีกครั้ง
พร้อมระบุ baseline ที่ผู้ใช้ยืนยันเข้าไปตรงๆ:

```bash
node .claude/skills/template-update/scripts/check-updates.mjs --baseline=0.7.0-template
```

script จะข้ามการเดาอัตโนมัติแล้วใช้เวอร์ชันที่ผู้ใช้ระบุเป็น baseline แทน
(ยังคงเช็คไขว้กับเนื้อไฟล์จริงให้เหมือนเดิม เผื่อผู้ใช้จำเวอร์ชันผิด) ถ้า
ผู้ใช้ไม่แน่ใจ/จำไม่ได้เลย ค่อยใช้ตัวเลขที่ script เดาจากเนื้อไฟล์จริง
("ใกล้เคียงกับ ... มากที่สุด") เป็นข้อมูลประกอบการตัดสินใจแทน
...
สำหรับไฟล์ที่ modified และไม่ใช่ 1 ใน config point ที่ `apply-config.mjs`
รู้จักอยู่แล้ว (`.gitlab-ci.yml`, `package.json`, `bun.lock`) script จะพิมพ์
เนื้อ diff เต็มของไฟล์นั้นออกมาให้ด้วย (ไม่ใช่แค่ชื่อไฟล์) — diff นี้เทียบ
**ไฟล์จริงของโปรเจกต์ตอนนี้ (`HEAD`) กับ template เวอร์ชันล่าสุด** ไม่ใช่
baseline เทียบกับ latest เฉย ๆ เพื่อให้เห็นของจริงที่จะเปลี่ยนถ้า merge ตอนนี้
รวมถึงกรณีที่โปรเจกต์เคยแก้ไฟล์นี้เอง (custom/hotfix) ไปแล้วตั้งแต่ merge
ครั้งก่อนด้วย **ต้องอ่านเนื้อ
diff นี้จริง ๆ** แล้วสรุปว่าเปลี่ยนอะไรเชิงโครงสร้าง/โค้ด (เช่น extend
class ใหม่, เปลี่ยน function signature, เพิ่ม dependency ใหม่) ถ้าเห็นว่า
โปรเจกต์นี้น่าจะมีโค้ดของตัวเองที่เกี่ยวข้องอยู่แล้ว (เช่นมี custom health
check ของตัวเอง แล้ว template เปลี่ยนให้ extend base class ใหม่) ให้
**เสนอแนะและถามผู้ใช้อย่างชัดเจนว่าต้องการปรับโค้ดของโปรเจกต์ให้สอดคล้อง
ด้วยหรือไม่** — ขั้นนี้ยังเป็นแค่การอ่าน/เสนอแนะ ไม่แก้ไฟล์ใด ๆ ในโปรเจกต์
(ยังอยู่ใน Stage A) ถ้าผู้ใช้ตอบว่าต้องการให้ปรับ ให้ทำ **แยกต่างหาก**
จากขั้นตอน merge/reapply-config ปกติ (เพราะเป็นการแก้โค้ดตาม logic ไม่ใช่
การ reapply ค่า config) และต้องให้ผู้ใช้ตรวจ `git diff` ก่อน commit เหมือนเดิม
