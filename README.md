...
PR ในโปรเจกต์นี้เอง) — บอก Claude ไปว่าล่าสุดที่ merge/อัปเดต template มา
จริง ๆ เป็นเวอร์ชันอะไร (ถ้าจำได้) แล้วให้รัน `check-updates.mjs
--baseline=X.Y.Z-template` ตามเวอร์ชันนั้นแทนการเดา ไม่ต้อง scaffold ใหม่
...
ทั้งสองเวอร์ชันพร้อมจำนวนบรรทัดที่ต่างกัน — Claude จะถามคุณตรงๆ ว่าล่าสุด
merge มาเป็นเวอร์ชันอะไร แล้วรันใหม่พร้อม `--baseline=X.Y.Z-template` ตามที่
คุณยืนยัน แทนที่จะเชื่อ auto-detect เพียงอย่างเดียว ถ้าไม่เจอคำเตือนนี้
แปลว่า baseline ที่เจอจาก commit message น่าเชื่อถือ ไม่ต้องยืนยันเพิ่ม
